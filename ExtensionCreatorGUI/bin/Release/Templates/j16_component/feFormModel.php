...
class <#= Value("Extension.name") #>Model<#= StrConv(Value("Task.nameObject"), VbStrConv.ProperCase) #> extends JModelAdmin
{
	...
	public function sendEmail($data)
	{
			...
			//Load the users to email
			$query	= $db->getQuery(true);
			$query->select('email');
			$query->from('#__users');
			$query->where('sendEmail = 1');
			$db->setQuery($query);	
			//TODO: Change loadResultArray() to loadColumn(), when its available
			...
			if(isset($data['email']) && $data['email']) {
				$subject	= 'Appointment Request Details';
	
				// Prepare email body
				//TODO: Fill in the response to user.
